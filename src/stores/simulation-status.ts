    ...
    persist(
        (set, get) => ({
            ...
            next() {
                ...
                const incompleteProcesses = state.processes.filter((process) => process.completionTime === null);
                            ...
                            processes: state.processes.map((process) => {
                                if (process.id === incompleteProcesses[0].id) {
                                    return {
                                        ...
                                        turnAroundTime: state.currentTime + process.burstTime - process.arrivalTime,
                                    };
                                }
                                return process;
                            }),
            ...
            previous() {
                set((state) => {
                    ...
                    return {
                        currentTime: state.currentTime - (lastProcess.end - lastProcess.start),
                        processes: state.processes.map((process) => {
