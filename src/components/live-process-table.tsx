...
export const LiveProcessView: React.FC<Props> = (props) => {
    ...
    return (
        <div className={cn(props.className)}>
            <Table>
                <TableCaption>{processes.length === 0 ? "Please start the simulation with some processes" : ""}</TableCaption>
                <TableHeader>
                    <TableRow>
                ...
                <TableBody>
                    {processes.map((process) => (
                        <TableRow className="h-14" key={process.id}>
                            ...
                            <TableCell>{process.turnAroundTime ? process.turnAroundTime - process.burstTime : "-"}</TableCell>
