...
export const GanttChart: React.FC<Props> = (props) => {
    ...
    return (<div
        className={cn(props.className, "rounded-lg flex items-center justify-center overflow-y-scroll gap-1")}
        ref={parent}
    >{
            ganttChart.length > 0 ? ganttChart.map((item) => {
                return (
                    <div
                        key={item.process || uuid.v4()}
                        className={`${item.process === "" ? "bg-red-400" : "bg-primary"} h-20 rounded-lg flex items-center justify-center`} style={{ width: `${(item.end - item.start) * 20}px` }}
                    >
