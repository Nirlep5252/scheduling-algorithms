...
import { useSimulationStatus, SimulationState } from "@/stores/simulation-status";
...
export const SimulationController: React.FC<Props> = (props) => {
    const { isRunning, setAlgorithm, pause, resume, start, next, previous, algorithm, isFinished, ganttChart } = useSimulationStatus();
        ...
        <div className={cn(props.className, "flex items-center justify-around")}>
            ...
            <div className="controls flex items-center justify-center gap-2">
                ...
                <Button
                    ...
                    className="w-12 h-12"
                >
                    {!isRunning ? <PlayIcon className="w-6 h-6" /> : <PauseIcon className="w-6 h-6" />}
                </Button>
                <Button
